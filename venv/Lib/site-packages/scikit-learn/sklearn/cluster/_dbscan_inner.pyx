...
def deep_dbscan_inner(np.ndarray[np.uint8_t, ndim=1, mode='c'] is_core,
    ...
    is_detected = np.full(ori_X.shape[0], 0, dtype=np.intp)
    ...
    for i in range(labels.shape[0]):
        ...
        while True:
            if labels[i] == -1 and is_core[i] and len(neighborhoods[i]) > min_samples:
                print('# labels[i] == -1 and core index : {}'.format(i))
                if hasPerson(ori_X[i], dc, i, is_detected):
                    print('## is person : {}'.format(i))
                    neighb = neighborhoods[i]
                    print('## neighborhoods : {}'.format(neighb))
                    detected = []
                    ...
                    if len(detected) >= min_samples:
                        ...
                        for j in range(len(detected)):
                            index = detected[j]
                            print('label : {}, index:{}'.format(labels[index], index))
                            if labels[index] == -1:
                                if is_core[index]:
                                    ...
                                    print('not core')
                                    print('label : {}, index:{}'.format(label_num, index))
                                    labels[index] = label_num
                            print('label : {}, index:{}'.format(labels[index], index))
                    # core 아니면 label 할당
                    else :
                        print('is not core, label_num : {}'.format(label_num))
                        labels[i] = label_num
                # Not Person
                else:
                    is_core[i] = 0
                ...
                print('label : {}, index:{}'.format(label_num, i))
                labels[i] = label_num
